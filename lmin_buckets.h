...
bucket_kmers find_l_minimizer_edge(char_view_t& kmers, edge_offset_t k, edge_offset_t l, vtx_view_t lmin_bucket_map){
    ...
    ordinal_t total_crosscut = 0;
    vtx_subview_t tc_sub = Kokkos::subview(cross_buckets, large_buckets*large_buckets);
    Kokkos::deep_copy(total_crosscut, tc_sub);
    ...
    Kokkos::parallel_for("partition by lmins", size, KOKKOS_LAMBDA(const ordinal_t i){
        ...
        lmin_in = lmin_in & large_buckets_mask;
        if(lmin_in != lmin_out){
            ordinal_t insert = cross_buckets(lmin_out) + Kokkos::atomic_fetch_add(&cross_buckets_count(lmin_out), 1);
            cross_writes(insert) = i;
            insert = cross_buckets(lmin_in) + Kokkos::atomic_fetch_add(&cross_buckets_count(lmin_in), 1);
            cross_writes(insert) = i;
        }
    });
    printf("Partitioned kmers in %.3f seconds\n", t.seconds());
