...
graph_t collect_outputs(graph_t glue_old, vtx_view_t nulls){
    ...
    graph_t output_g(writes, write_sizes);
...
graph_t collect_unitigs(graph_t glue_old, graph_t glue_action){
    edge_offset_t write_size = 0;
    ...
    Kokkos::parallel_scan("count entries", policy_t(0, n), KOKKOS_LAMBDA(const ordinal_t u, edge_offset_t& update, const bool final){
        ...
        for(edge_offset_t i = glue_action.row_map(u); i < glue_action.row_map(u + 1); i++){
            ordinal_t f = abs(glue_action.entries(i)) - 1;
    ...
    graph_t output_g(writes, next_offsets);
    //number of columns is not accurate, but is not used either
    return output_g;
}
